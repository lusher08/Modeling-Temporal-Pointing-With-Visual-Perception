...
class FlashClickEnv(gym.Env):
    def __init__(self, T_true=1.0, dt=0.02, tol=0.15, max_steps=3000):
        ...
        # ======= CHANGED: motor delay (push mean and sigma up to encourage right skew) =======
        self.motor_mu = 0.27    # was 0.2
        self.motor_sigma = 0.09 # was 0.07

        # 탐색 보장 (무조건 클릭을 해야하도록)
        self.min_click_prob = 0.05

        # vigilant scale: reduce the strength of "speeding up" during vigilant
        self.vigilant_scale = 0.8  # introduced variable; used below (was implicit 0.6)
    ...
    def set_predicted_interval(self, pred_interval):
        pred_interval = float(np.clip(pred_interval, 0.05, 2.0))
    ...
    def _match_click(self, actual_click_time):
        # 허용 전/후 윈도우 (초)
        forward_allow = 0.1  # 앞으로 0.1s 이내면 다음 플래시에 "anticipation"으로 매칭 허용
        backward_allow = self.match_window  # 기존 매칭 윈도우를 후방 허용으로 사용

        # 과거 플래시들만 고려 (actual_click_time 기준으로 이전 또는 동일)
        past_candidates = [ft for ft in self.flash_times if ft <= actual_click_time and (actual_click_time - ft) <= backward_allow]
        if len(past_candidates) > 0:
            # 가장 최근(closest past) 플래시 선택
            matched = max(past_candidates)
            return matched

        # 과거 플래시가 없으면, 다음 플래시(미래)와의 거리가 작을 때만 허용
        if abs(actual_click_time - self.next_flash_time) <= forward_allow:
            return self.next_flash_time
    ...
    def step(self, action):
        ...
        # ======= CHANGED: use updated mu/sigma and clamp negatives =======
        motor_delay = np.random.normal(self.motor_mu, self.motor_sigma)
        motor_delay = max(motor_delay, 0.0)
        if self.vigilant_remaining > 0:
            # ======= CHANGED: make vigilant effect less aggressive (0.8 instead of 0.6) =======
            motor_delay *= self.vigilant_scale

        # 최소 클릭 확률 보장
        if np.random.rand() < self.min_click_prob:
            # 내부 시계가 존재하고 거의 끝나갈 때만 강제 클릭, 아니라면 무시
            if self.internal_clock_remaining is None or self.internal_clock_remaining <= 0.05:
                action = 1
        ...
        if int(action) == 1:
            ...
            if matched_flash is not None:
                ...
                if delta < 0:
                    # ======= CHANGED: stronger penalty for early clicks (more negative) =======
                    # previous: reward = -0.1 - 0.1 * abs(delta)
                    # now: make early clicks strongly discouraged to shift mean right
                    reward = np.exp(delta / 0.25) - 0.4
                else:
                    tau = 0.25
...
def train():
    ...
    env = FlashClickEnv(T_true=1.0, max_steps=3000)  # you can randomize per-episode if desired
    ...
    for episode in range(max_episodes):
        ...
        # 에피소드마다 데이터 저장 (한 플래시당 한 클릭)
        flash_to_click = {}
        for ft in env.flash_times:
            candidates = [ct for (f, ct) in env.click_to_flash_map
                          if f is not None and abs(ct - ft) <= env.match_window]
            if len(candidates) > 0:
                chosen = min(candidates, key=lambda x: abs(x - ft))
                flash_to_click[ft] = chosen
        with open("model_data.csv", "a", newline="") as f:
            writer = csv.writer(f)
            for ft in sorted(flash_to_click.keys()):
                writer.writerow([ft, flash_to_click[ft]])

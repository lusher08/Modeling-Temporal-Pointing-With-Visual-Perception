...
class FlashClickEnv(gym.Env):
    def __init__(self, T_true=1.0, WT=0.2, dt=0.6, tol=0.15, max_steps=3000):
        super().__init__()
        self.T_true = T_true
        self.WT = WT
        self.dt = dt
        ...
        # motor noise
        self.motor_mu = 0.27
        self.motor_sigma = 0.09

        self.min_click_prob = 0.05

        # wt를 적용하고자, wt가 낮아지면 vigilance의 버프를 덜 받도록 설정함
        '''
        - dt 영향력 : dt가 증가함에 따라 윈도우 크기도 증가 > 분산 증가 > 정확도 떨어짐 효과
                    (따라서 별도 구현 없음)
        - wt 영향력 : wt가 짧아지면 vigilance의 버프를 덜 받도록 설정(기본적인 정확도 감소 효과)
                     반복함에 따라 학습에 의해 vigilance 의존도가 낮아짐 > 내부시계에 더 의존하게된다
        '''
        WT_norm = np.clip((0.2 - self.WT), 0.0, 0.2) / 0.2
        self.vigilant_scale = 0.4 + 0.6 * WT_norm
    ...
    def step(self, action):
        ...
        if int(action) == 1:
            ...
            if matched_flash is not None:
                bias = 0.05
                delta = actual_click_time + bias - matched_flash
                if delta < 0:
